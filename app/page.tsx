...
import GradientCanvas, { Point } from "./_components/GradientCanvas";
...
export default function Home() {
  ...
  const [points, setPoints] = useState<Point[]>([]); // Add proper typing
  const [isClicked, setIsClicked] = useState(false);

  const audio = useRef<HTMLAudioElement | null>(null);

  useEffect(() => {
    const audioElement = new Audio("/audio/Blue.mp3");
  ...
  return (
    <>
      <main
        className="flex min-h-screen flex-col items-center justify-center p-24 cursor-none"
        onClick={startMusic}
        onKeyDown={(e) => e.key === " " && startMusic()}
        tabIndex={0}
        role="button"
